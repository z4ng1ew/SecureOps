...
bool check_options(int num_args, char* av[],
                   Struct_singl_lett_opt* single_letter_opt) {
  bool valid_flag = true;
  int option, option_index = 0;
  while ((option = getopt_long(num_args, av, "beEnstTv", long_options,
                               &option_index)) != -1 &&
         valid_flag) {
    switch (option) {
      case 'b':
...
void handle_file(Struct_singl_lett_opt single_letter_opt,
                 const char* filename) {
...
void process_current_character(int current_character, int* last_char,
                               const Struct_singl_lett_opt single_letter_opt,
                               int* counter_consecutive_empty_lines_s,
                               int* number_current_lines_b_n) {
  ...
  if (*last_char == '\n' &&
      (single_letter_opt.fl_n ||
       (single_letter_opt.fl_b && current_character != '\n'))) {
  ...
  if (single_letter_opt.fl_v && current_character != '\t' &&
      current_character != '\n') {
    ...
    if (current_character < 32 || current_character == 127) {
      printf("^");
      current_character = (current_character == 127)
                              ? current_character - 128 + 64
                              : current_character + 64;
    }
  }
...
void print_file_with_options(FILE* file,
                             const Struct_singl_lett_opt single_letter_opt) {
  static int current_character, last_char = '\n';
  static int counter_consecutive_empty_lines_s = 0,
             number_current_lines_b_n = 0;
  while ((current_character = fgetc(file)) != EOF) {
    process_current_character(current_character, &last_char, single_letter_opt,
                              &counter_consecutive_empty_lines_s,
                              &number_current_lines_b_n);
  }
}
