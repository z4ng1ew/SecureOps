...
int main(int ac, char *av[]) {
  ...
  if (GrepParams.has_errors) {
    fprintf(
        stderr,
        "Распишите команду по структуре: grep [ПАРАМЕТР]… ШАБЛОНЫ [ФАЙЛ]…\n");
  }

  return GrepParams.has_errors;
}

void initialize_and_process(int ac, char *av[], Letter_opt *CmdFlags,
                            Retrieval_Config *GrepParams, char **pattern) {
...
void initialize_flags(Letter_opt *CmdFlags) { *CmdFlags = (Letter_opt){false}; }
...
void process_flag_f(Letter_opt *CmdFlags, Retrieval_Config *settings,
                    char **pattern, char *optarg) {
  CmdFlags->f_fl = true;
  settings->processed_document_count++;
  import_patterns_from_file(settings, pattern, optarg);
}

void set_flags(int option, Letter_opt *CmdFlags, Retrieval_Config *settings,
               char **pattern) {
  switch (option) {
    case 'e':
...
void analyze_cmd_arg(int ac, char *av[], Letter_opt *CmdFlags,
                     Retrieval_Config *settings, char **pattern) {
  initialize_flags(CmdFlags);

  int option;
  while ((option = getopt(ac, av, "e:ivclnhsf:o")) != -1 &&
         !(settings->has_errors)) {
...
void import_patterns_from_file(Retrieval_Config *GrepParams, char **pattern,
                               char *optarg) {
...
void scan_file(Letter_opt CmdFlags, Retrieval_Config *GrepParams,
...
void validate_and_compile_regex(Letter_opt CmdFlags,
                                Retrieval_Config *GrepParams, regex_t *regex,
                                char *pattern) {
...
void handle_opt_o(Letter_opt CmdFlags, Retrieval_Config *GrepParams, FILE *file,
  ...
  while ((getline(&line, &length, file)) != -1) {
    ...
    while (((status = regexec(regex, copy_line, 1, &match, REG_NOTEOL)) == 0) &&
      ...
      if (GrepParams->processing_a_lot_of_files && !CmdFlags.h_fl)
        printf("%s:", filename);
...
void handle_matching_regex(Letter_opt CmdFlags, Retrieval_Config *GrepParams,
                           FILE *file, regex_t *regex, const char *filename) {
  char *line = NULL;
  size_t length = 0;
